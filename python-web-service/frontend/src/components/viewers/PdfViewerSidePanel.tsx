...
export default function PdfViewerSidePanel({
    ...
    const [searchResults, setSearchResults] = useState<Array<{ page: number; highlight: string; id: string; occurrenceIndex: number }>>([]);
    const [addedAnnotationIds, setAddedAnnotationIds] = useState<Array<{ pageNumber: number; annotationId: string }>>([]);
        ...
        try {
            ...
            if (formattedResults.length > 0) {
                ...
                const firstAnnotation = viewer.annotationCollection?.find(
                    (a: any) => a.customData?.searchId === firstResult.id
                );
                if (firstAnnotation) {
                    setTimeout(() => {
            ...
            addedAnnotationIds.forEach((item) => {
                try {
                    const annotation = viewer.annotationCollection?.find(
                        (a: any) => a.customData?.searchId === item.annotationId
                    );
                    if (annotation) {
    ...
    const handleOccurrenceChange = (event: { target: { value: string } }) => {
